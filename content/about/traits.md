<details name="traits">
<summary>⚖️Pragmatic</summary>


I’d rather be accurately dissed than impressively praised.

I value introspection over bragging, and useful feedback over empty praise.<br>Compliments are nice. Constructive criticism is better.

<br>

</details>

<details name="traits">
<summary>🎯Direct</summary>


I tend to look for the actual point and go there.

I’m usually positive, polite and fairly reserved<br>but I don’t have much patience for euphemistic choreography<br>or performative political correctness.

I prefer wrapping difficult things in constructive feedback, not cotton wool.

<br>

</details>

<details name="traits">
<summary>⚡Dopamine-driven</summary>

I like momentum, novelty and fast feedback.<br>Ceremonies, unnecessary waiting and stretched information are my nemesis.

I tend to consume and communicate knowledge in compressed capsules:<br>separate ideas, clear structure, diagrams and text that is easy to scan.

<br>

</details>

<details name="traits">
<summary>🔬Curious Engineer</summary>


“It works because it works” has never been a satisfying explanation for me.<br>I want to see the gears and understand underlying principles.

Finding shortcuts and model improvements is my background process.<br>I can talk about science and analysis for an unreasonable amount of time.

<br>

</details>

<details name="traits">
<summary>🤖Tech-collecting Nerd</summary>


Being a programmer feels more like a personality leak than a job.

I collect gadgets, tools and technologies, then immediately start asking:<br>How does this work? What can it do? And what else can it do?

Apparently this counts as a hobby.

<br>

</details>

<details name="traits">
<summary>🧩Autistic Introvert</summary>


Text beats voice. Small groups beat crowds.

I like people - just not necessarily all of them at once.<br>I’m prefer asynchronous communication that gives me space to think.<br>

<br>

</details>

<details name="traits">
<summary>✨Enthusiastic</summary>


My default setting is usually positive, curious and engaged.

And when something genuinely catches my attention,<br>“engaged” escalates into slightly unreasonable levels of focus very quickly.

</details>
